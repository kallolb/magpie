...
- `app/routers/` — API endpoints: `videos`, `downloads`, `tags`, `categories`, `webhook`, `settings`, `loop_markers`, `analytics`, `compilations`
- `app/services/` — Business logic: `downloader` (yt-dlp), `search` (FTS5), `categorizer`, `thumbnail`, `notifier`, `renderer` (ffmpeg compilation)
- `app/models/` — Pydantic models for `video`, `tag`, `category`, `loop_marker`, `compilation`
...
- `src/pages/` — Route pages: Dashboard, Browse, Download, VideoView, Search, Analytics, Compilations, CompilationEditor, Settings
...
- Frontend uses Tailwind CSS for styling (no CSS modules), recharts for analytics, and @dnd-kit for drag-and-drop
- Loop markers are stored in the `loop_markers` table and managed via `/api/videos/{id}/loops` endpoints
- Compilations use `compilations` + `compilation_clips` tables; rendered via ffmpeg (stream copy or re-encode)
- Compilation clips reference source videos; deletion check via `GET /api/videos/{id}/deletion-check`
